    ...
    <div className="max-w-4xl mx-auto px-5 py-12">
      ...
      <section className="mb-12">
        ...
        {enrollList.length === 0 ? (
          ...
          <div className="space-y-3">
            {enrollList.map((e) => (
              <div key={e.id} className="p-4 border border-kb-green/15 rounded-xl flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-sm text-kb-charcoal/60">
                    <a href={`mailto:${e.email}`} className="hover:text-kb-green">{e.email}</a> {e.phone ? `· ${e.phone}` : ""}
                  </p>
                  <p className="text-xs text-kb-charcoal/40 mt-1">{new Date(e.created_at).toLocaleString()}</p>
                </div>
                <StatusSelect table="enrollments" id={e.id} value={e.status} options={REQUEST_STATUSES} />
              </div>
            ))}
          </div>
        )}
      ...
      <section className="mb-12">
        ...
        {bookingList.length === 0 ? (
          ...
          <div className="space-y-3">
            {bookingList.map((b) => (
              <div key={b.id} className="p-4 border border-kb-green/15 rounded-xl flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  <p className="font-medium text-kb-charcoal">
                    {b.full_name} <span className="text-kb-charcoal/40 font-normal">— {b.service?.title}</span>
                  </p>
                  <p className="text-sm text-kb-charcoal/60">
                    <a href={`mailto:${b.email}`} className="hover:text-kb-green">{b.email}</a> {b.phone ? `· ${b.phone}` : ""}
                    {b.company ? ` · ${b.company}` : ""}
                  </p>
                  {b.preferred_date && (
                    <p className="text-xs text-kb-charcoal/50 mt-1">Preferred date: {b.preferred_date}</p>
                  )}
                  {b.message && <p className="text-sm text-kb-charcoal/70 mt-1 whitespace-pre-wrap">{b.message}</p>}
                  <p className="text-xs text-kb-charcoal/40 mt-1">{new Date(b.created_at).toLocaleString()}</p>
                </div>
                <StatusSelect table="consulting_bookings" id={b.id} value={b.status} options={REQUEST_STATUSES} />
              </div>
            ))}
          </div>
        )}
      ...
      <section>
        ...
        {inquiryList.length === 0 ? (
          ...
          <div className="space-y-3">
            {inquiryList.map((i) => (
              <div key={i.id} className="p-4 border border-kb-green/15 rounded-xl flex flex-wrap items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-kb-charcoal/60">
                    <a href={`mailto:${i.email}`} className="hover:text-kb-green">{i.email}</a> {i.phone ? `· ${i.phone}` : ""}
                  </p>
                  <p className="text-sm text-kb-charcoal/70 mt-1 whitespace-pre-wrap">{i.message}</p>
                  <p className="text-xs text-kb-charcoal/40 mt-1">{new Date(i.created_at).toLocaleString()}</p>
                </div>
                <StatusSelect table="inquiries" id={i.id} value={i.status} options={INQUIRY_STATUSES} />
              </div>
            ))}
          </div>
        )}
